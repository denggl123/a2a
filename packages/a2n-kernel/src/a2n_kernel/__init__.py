"""a2n-kernel（L0 内核）：所有包都能依赖它，它不依赖任何包。
    ...
    errors     领域异常基类（A2NError → 400/404/409）
    protocols  跨包契约小工具（鸭子形态校验 attrs）

这里不允许出现任何业务概念：不认识"任务"，也不认识"钱"。
"""
from .errors import A2NError, ConflictError, NotFoundError, ValidationError
...
__all__ = ["publish", "subscribe", "GENESIS", "canonical_json", "chain_hash",
           ...
           "A2NError", "ValidationError", "NotFoundError", "ConflictError",
           "attrs"]
