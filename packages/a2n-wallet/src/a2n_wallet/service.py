...
class Wallet:
    ...
    def request(self, account_id: str, amount: int) -> dict[str, Any]:
        ...
        self.ledger.post(account_id, -amount, "withdraw_freeze", wd_id)
        self.ledger.post(hold, amount, "withdraw_freeze", wd_id)
        conn().execute(
    ...
    def mark_paid(self, withdrawal_id: str, custodian_ref: str) -> dict[str, Any]:
        r = conn().execute("SELECT * FROM withdrawals WHERE id=?", (withdrawal_id,)).fetchone()
        conn().execute("UPDATE withdrawals SET state='PAID', custodian_ref=? WHERE id=?",
