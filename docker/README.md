...
| `docker/market_node.py` | 「找 Agent」里的**行业案例**（短视频 / 财务 / 法务 / 游戏 / 电商），一台容器跑两份供给 | **故意不通**：卡是真的、网络是通的，但夹具没接真实 agent |

案例那一路的定位要说清：**控制台 → 平台 → relay 入口 → 反向隧道 → 容器内本地服务**
整段是真生产链路（注册、心跳、地址投影、门禁、拨号一处不少），只有容器里
"这活到底干得了吗"这一跳按设计答"干不了"。所以它验的不是"调得通"，
而是**调不通时有没有如实说** —— 失败原因必须是容器自己那句话，
不许被换成 `upstream 500` 这种通用噪声。

## 常规用法：跟着演示环境起来

```bash
bash scripts/sim_start.sh fresh     # 平台（0.0.0.0:8000）+ 四节点 + 三个案例容器
```
...
docker run -d --name a2n-market-video --add-host host.docker.internal:host-gateway \
  ...
  -e A2N_CONTAINER=video-studio -e A2N_PRINCIPAL=acct:alice \
...
docker run -d --name a2n-ocr --add-host host.docker.internal:host-gateway \
  ...
  -e A2N_PRINCIPAL=acct:d-ocr -e A2N_REGION=cn-docker-a \
  a2n-agent
...
| `A2N_PRINCIPAL` | `acct:alice` | 主体（默认与控制台开箱主体一致） |
| `A2N_PORT_BASE` | `8787` | 容器内本地服务起始端口（两组各占一个） |
...
**二、钥匙要落盘在卷里**，不能每次启动现生成。uid 由钥匙派生，换了钥匙
...
- 控制台「找 Agent」里与四节点并列可见，卡自证 `signed`、`reachable=True`；
- 调它一笔（免费档，门禁放行）→ 任务 **FAILED**，且失败原因是容器自己那句话
  （"本容器是演示夹具：这张卡上的能力没有接真实 agent…"），
...
A2N_PRINCIPAL=acct:p-ocr A2N_PORT_BASE=8781 python -u docker/market_node.py
