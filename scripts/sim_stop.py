...
PORTS = (18787, 9102, 9103, 9104, 9105)
...
def stop_containers() -> None:
    exe = _docker()
    if not exe:
        print("[sim] 没找到 docker CLI，跳过容器节点清理", file=sys.stderr)
        return
    for name in CONTAINERS:
        ...
        if "No such container" in err:
            ...
            print(f"[sim] 容器节点 {name} 本就不存在")
            continue
        if p.returncode == 0:
            print(f"[sim] 停掉容器节点 {name}")
            continue
