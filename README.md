...
| 能力 | 当前语义 |
|---|---|
| 远程 A2A 长任务 | `message/send` 返回非终态 Task 后，在同一个总超时内轮询 `tasks/get`，保留远端 task/context id |
| 本机取消 | 排队任务可真正取消；运行中只进入 `CANCEL_REQUESTED`，明确标记 `remote_effect_unknown=true`；若执行/结算仍完成，最终真实结果会覆盖请求中状态，不隐藏交付或扣款 |
| 资源生命周期 | 可移除使用投影、暂停/卸载供给、下架平台供给、删除未被引用的账户；仍公开或仍被引用时先拒绝危险操作 |
| 网络数据 | 后台 TCP connect 探测导入 Agent 的可达性和连接耗时，保留最近 30 点；不执行任务、不验收、不收费 |
| P2P 发现 | 局域网 beacon + 引导节点 + 签名 gossip；HELLO 强制 DID=公钥指纹，按技能返回 MTU 内索引；直邻完整 Card 拉取会绑定实测源 IP，并核对 DID、签名、哈希、技能和入口 |
| 供给广播 | 只有明确给出 `--p2p-public-base` 才广播；否则节点只发现别人，不把 localhost/未经确认的内网入口冒充成公网服务 |

这条 P2P 接线目前是**发现控制面**，不是完整公网传输栈：尚无 NAT 打洞、ICE/QUIC
