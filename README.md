...
# 用户只打开平台控制台：http://127.0.0.1:8000/console
# 8771 是本机 SDK / A2A 后台入口，由平台控制台中的“本机 Agent”面板管理
```

`8000/console` 是唯一产品界面。`8771` 必须独立存在，因为它运行在每位用户自己的
电脑上，保存本地账户并转发本地 Agent；把它并进平台服务就会让平台接管用户凭据，也会
让平台离线时本机工作台一起失效。但这个进程边界不应变成第二套使用体验：账户、供给、
工作台投影和网络状态都在平台控制台的“本机 Agent”面板里配置。`8771/console` 只保留为
开发诊断页，不作为正常入口。
